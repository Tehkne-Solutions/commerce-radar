(() => {
  ...
  const KEYS = {
    monitors: 'tehkne-commerce-radar-v83-evidence-prevention-monitors',
    checkpoints: 'tehkne-commerce-radar-v83-evidence-prevention-checkpoints',
    events: 'tehkne-commerce-radar-v83-evidence-prevention-events',
    settings: 'tehkne-commerce-radar-v83-evidence-prevention-settings',
    snapshots: 'tehkne-commerce-radar-v83-evidence-prevention-snapshots',
    reports: 'tehkne-commerce-radar-v83-evidence-prevention-reports',
  };
  ...
  function startMonitoring(experimentId, source = {}) {
    ...
    checkpoint(experimentId, { score: baselineScore, componentScores: source.componentScores || {}, reason: 'monitoring_started' });
  ...
  function evaluate(monitorId, latestCheckpoint = null) {
    ...
    const consecutiveDrops = series.slice(-4).every((row, index, rows) => index === 0 || row.score < rows[index - 1].score);
    let status = 'monitoring';
    if (loss >= settings().recurrenceLoss) status = 'recurrent';
    else if (loss >= settings().attentionLoss || (series.length >= 4 && consecutiveDrops)) status = 'attention';
  ...
  function report() {
    const rows = history();
    const stable = rows.filter(row => row.status === 'stable').length;
    const recurrent = rows.filter(row => row.status === 'recurrent').length;
    const completed = stable + recurrent;
    return { total: rows.length, monitoring: rows.filter(row => row.status === 'monitoring').length, attention: rows.filter(row => row.status === 'attention').length, stable, recurrent, successRate: completed ? Math.round((stable / completed) * 1000) / 10 : 0, rows };
  }
  function captureSnapshot(reference = today()) { const row = { id: `prevention-snapshot-${reference}`, date: reference, summary: report(), capturedAt: nowIso(), signature: 'Tehkné Solutions' }; write(KEYS.snapshots, [row, ...snapshots().filter(item => item.date !== reference)].slice(0, settings().keepSnapshots)); return row; }
