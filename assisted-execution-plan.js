(() => {
  ...
  const sourceRuns=()=>ROOT.CommerceRadarBudgetOptimizer?.runs?.()||[];
  ...
  function createPlan(runId){
    const run=sourceRuns().find(x=>x.id===runId)||sourceRuns()[0];
    ...
    const tasks=buildTasks(run);
    const row={id:`assisted-plan-${uid()}`,runId:run.id,strategy:run.strategy,budget:run.budget,constraints:run.constraints,allocations:run.allocations.map(a=>({id:a.id,title:a.title,amount:a.amount,weight:a.weight,channel:a.channel,product:a.product,supplier:a.supplier})),tasks,createdAt:nowIso(),signature:'Tehkné Solutions',safety:'assistido-sem-execucao-automatica'};
  ...
  function updateTask(planId,taskId,status,note='',owner){
    ...
    task.status=status;task.note=String(note).slice(0,500);if(owner)task.owner=String(owner).slice(0,120);task.updatedAt=nowIso();
    write(KEYS.plans,all);
    const event={id:`assisted-event-${uid()}`,planId,taskId,status,note:task.note,owner:task.owner,createdAt:nowIso(),signature:'Tehkné Solutions'};
  ...
  ROOT.CommerceRadarAssistedExecutionPlan={KEYS,PHASES,STATUS,plans,buildTasks,createPlan,updateTask,summary,captureSnapshot,exportMarkdown,money};extendCloud();
})();