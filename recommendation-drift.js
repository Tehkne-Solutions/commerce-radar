(() => {
  ...
  const KEYS = {
    ...
    controlHistory: 'tehkne-commerce-radar-v63-profile-control-history',
  ...
  const STATUS = {
    ...
    inactive: { label: 'Sem aplicação', recommendation: 'Ativar ou simular um perfil para iniciar o monitoramento.' },
  };
  ...
  const addDays = (value, days) => { const date = new Date(`${iso(value || today())}T12:00:00`); date.setDate(date.getDate() + Number(days || 0)); return date.toISOString().slice(0, 10); };
  ...
  function auditOutcome(row = {}) {
    ...
    const costs = ['productCost', 'marketplaceFees', 'paymentFees', 'taxes', 'advertising', 'packaging', 'otherCosts'].reduce((sum, key) => sum + Math.max(0, num(row[key])), shipping);
    const profit = Number.isFinite(Number(row.netProfit)) ? Number(row.netProfit) : netSales - costs;
  ...
  function outcomeFor(row, input = sourceInput(), config = settings(), reference = today()) {
    ...
    const tests = (input.tests || []).filter((item) => normalize(item.product) === key && eventDate(item) > start && eventDate(item) <= end);
    const audits = (input.audits || []).filter((item) => normalize(item.product) === key && eventDate(item) > start && eventDate(item) <= end);
    ...
    const profitable = audits.some((item) => { const result = auditOutcome(item); return result.profit > 0 && result.margin >= 8; });
  ...
  function dedupeControlledRows(rawSnapshots = controlSnapshots(), config = settings(), reference = today()) {
    ...
    for (const snapshot of rawSnapshots) {
      ...
      for (const item of snapshot.ranking || []) {
        ...
        const week = (() => { const date = new Date(`${snapshot.date}T12:00:00`); const day = date.getDay() || 7; date.setDate(date.getDate() - day + 1); return date.toISOString().slice(0, 10); })();
        rows.push({ ...item, date: snapshot.date, snapshotId: snapshot.id, week, profileId: item.profileId, controlledScore: num(item.score), globalScore: num(item.globalScore), product: item.product, key: item.key || normalize(item.product) });
      }
    }
    const latest = new Map();
    for (const row of rows.sort((a, b) => String(a.date).localeCompare(String(b.date)))) latest.set(`${row.profileId}:${row.key}:${row.week}`, row);
  ...
  function buildProfileReports(rawSnapshots = controlSnapshots(), input = sourceInput(), config = settings(), reference = today(), allProfiles = profiles()) {
    const rows = dedupeControlledRows(rawSnapshots, config, reference).map((row) => ({ ...row, outcome: outcomeFor(row, input, config, reference) }));
    ...
    return [...grouped.entries()].map(([profileId, cases]) => {
      ...
      const pending = cases.filter((row) => row.outcome.status === 'pending').length;
      const inconclusive = cases.filter((row) => row.outcome.status === 'inconclusive').length;
      return {
        profile, cases, controlled, global, pending, inconclusive, status,
    ...
    }).sort((a, b) => ({ critical: 0, warning: 1, insufficient: 2, stable: 3 }[a.status] - ({ critical: 0, warning: 1, insufficient: 2, stable: 3 }[b.status]) || b.controlled.total - a.controlled.total);
  }
  ...
  function overallReport(reports = buildProfileReports()) {
    ...
    return { profiles: reports.length, conclusive, critical, warning, stable, status: critical ? 'critical' : warning ? 'warning' : stable ? 'stable' : 'insufficient' };
  }
  ...
  function captureDriftSnapshot(reference = today(), force = false) {
    ...
    const row = {
      id: `profile-drift-${reference}`, date: reference, config,
      overall: overallReport(reports),
      profiles: reports.map((item) => ({ profileId: item.profile.id, label: item.profile.label, dimension: item.profile.dimension, status: item.status, controlled: item.controlled, global: item.global, accuracyDelta: item.accuracyDelta, brierDelta: item.brierDelta, pending: item.pending, inconclusive: item.inconclusive, recommendation: item.recommendation })),
      createdAt: new Date().toISOString(), signature: 'Tehkné Solutions',
    };
  ...
  function saveReview(profileId, decision, note = '') {
    const row = { id: `drift-review-${uid()}`, profileId, decision, note: safe(note, 1000), at: new Date().toISOString(), signature: 'Tehkné Solutions' };
  ...
  function toast(message, error = false) {
    ...
    if (!node && typeof document !== 'undefined') { node = document.createElement('div'); node.id = 'profileDriftToast'; document.body.append(node); }
    if (!node) return;
  ...
  function render() {
    ...
    const count = $('profileDriftNavCount'); if (count) count.textContent = overall.critical + overall.warning ? String(overall.critical + overall.warning) : '';
    ...
    list?.querySelectorAll('[data-drift-review]').forEach((button) => { button.onclick = () => { const note = prompt('Observação da revisão:', '') ?? ''; saveReview(button.dataset.driftReview, button.dataset.decision, note); toast('Revisão registrada sem alterar o ranking.'); render(); }; });
    const trend = $('profileDriftHistory');
    if (trend) trend.innerHTML = snapshots().slice(0, 14).map((row) => `<div class="driftHistoryRow"><span>${new Date(`${row.date}T12:00:00`).toLocaleDateString('pt-BR')}</span><b>${esc(STATUS[row.overall?.status || 'insufficient'].label)}</b><small>${row.overall?.conclusive || 0} casos · ${row.overall?.critical || 0} críticos</small></div>`).join('') || '<p class="muted">Capture o primeiro diagnóstico para formar o histórico.</p>';
    const reviewNode = $('profileDriftReviews');
    if (reviewNode) reviewNode.innerHTML = reviews().slice(0, 12).map((row) => `<div class="driftHistoryRow"><span>${new Date(row.at).toLocaleString('pt-BR')}</span><b>${esc(row.decision)}</b><small>${esc(row.note || '')}</small></div>`).join('') || '<p class="muted">Nenhuma revisão registrada.</p>';
    if ($('driftHorizon')) $('driftHorizon').value = config.horizonDays;
    if ($('driftLookback')) $('driftLookback').value = config.lookbackDays;
    if ($('driftMinimum')) $('driftMinimum').value = config.minimumSample;
  }
  ...
  function exportReport() {
    ...
    for (const row of reports) lines.push(`- ${row.profile.label}: ${STATUS[row.status].label}; acurácia perfil ${row.controlled.accuracy.toFixed(1)}%; global ${row.global.accuracy.toFixed(1)}%; diferença ${row.accuracyDelta.toFixed(1)} p.p.; Brier Δ ${row.brierDelta.toFixed(3)}; ${row.recommendation}`);
    ...
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = `commerce-radar-drift-${today()}.md`; anchor.click(); URL.revokeObjectURL(url);
  }
  ...
  function showView() {
    ...
    render(); globalThis.scrollTo?.({ top: 0, behavior: 'smooth' });
  }

  function extendCloud() {
    const apply = () => { const keys = ROOT.CommerceRadarCloud?.dataKeys; if (!keys) return false; keys.profileDriftSettings = KEYS.settings; keys.profileDriftSnapshots = KEYS.snapshots; keys.profileDriftReviews = KEYS.reviews; return true; };
  ...
  function enhanceBackup() {
    ...
    const timer = setInterval(() => {
      ...
      if (!backup || !input || !merge || !replace) { if (attempts > 340) clearInterval(timer); return; }
      clearInterval(timer);
      backup.onclick = () => { const keys = ROOT.CommerceRadarCloud?.dataKeys || {}; const payload = { version: '0.6.4', exportedAt: new Date().toISOString(), signature: 'Tehkné Solutions' }; for (const [name, key] of Object.entries(keys)) payload[name] = read(key, []); payload.profileDriftSettings = settings(); payload.profileDriftSnapshots = snapshots(); payload.profileDriftReviews = reviews(); const url = URL.createObjectURL(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `commerce-radar-backup-${today()}.json`; anchor.click(); URL.revokeObjectURL(url); };
      input.addEventListener('change', async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const payload = JSON.parse(await file.text()); pending = { settings: payload.profileDriftSettings || {}, snapshots: Array.isArray(payload.profileDriftSnapshots) ? payload.profileDriftSnapshots : [], reviews: Array.isArray(payload.profileDriftReviews) ? payload.profileDriftReviews : [] }; } catch { pending = { settings: {}, snapshots: [], reviews: [] }; } }, { capture: true });
      merge.addEventListener('click', () => { write(KEYS.settings, { ...settings(), ...pending.settings }); write(KEYS.snapshots, [...new Map([...snapshots(), ...pending.snapshots].map((item) => [item.id, item])).values()].slice(0, 180)); write(KEYS.reviews, [...new Map([...reviews(), ...pending.reviews].map((item) => [item.id, item])).values()].slice(0, 300)); render(); });
      replace.addEventListener('click', () => { write(KEYS.settings, pending.settings); write(KEYS.snapshots, pending.snapshots); write(KEYS.reviews, pending.reviews); render(); });
    }, 50);
  }

  function inject() {
    if (typeof document === 'undefined') return false;
    const controlNav = $('profileControlNav'); const controlView = $('profileControl');
    ...
    $('driftCapture').onclick = () => { captureDriftSnapshot(today(), true); toast('Diagnóstico de drift capturado.'); render(); };
    $('driftExport').onclick = exportReport;
    $('driftSaveSettings').onclick = () => { write(KEYS.settings, { ...settings(), horizonDays: num($('driftHorizon').value, 21), lookbackDays: num($('driftLookback').value, 90), minimumSample: Math.max(4, num($('driftMinimum').value, 6)) }); toast('Parâmetros atualizados.'); render(); };
    extendCloud(); enhanceBackup(); captureDriftSnapshot(); render();
  ...
  function boot() { if (inject()) return; let attempts = 0; const timer = setInterval(() => { attempts += 1; if (inject() || attempts > 480) clearInterval(timer); }, 50); }

  ROOT.CommerceRadarRecommendationDrift = { KEYS, DEFAULTS, outcomeFor, dedupeControlledRows, classificationMetrics, statusFor, buildProfileReports, overallReport, captureDriftSnapshot, saveReview };
